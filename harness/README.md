...
## 설치 전 심층 분석 + 모드 추천 (2026-07-07)

`install.sh`/`install.ps1`은 **설치를 시작하기 전에 대상 머신의 기존 하네스를 심층 분석**한다 — `CLAUDE.md`(줄수, 빈 줄 포함)·`rules`·`skills`·`agents`·`commands`·`settings.json` 존재를 스캔해 **모드를 추천**한다:
- 기존 하네스 **없음/최소** → **[완전 치환]**(깨끗한 신규 설치).
- **내 깃허브 하네스 replace 이전 설치 감지**(`.harness-source` 마커의 `mode=replace`) → **[완전 치환=업그레이드]**(최신본 클린 교체, 기존 백업).
- **내 하네스를 merge로 올린 이력**(마커 `mode=merge` = 커스텀+우리것 혼합) → **[개선(merge)]** 유지(커스텀 보존; replace면 커스텀이 활성에서 밀림).
- **외부/커스텀 하네스 감지**(마커 없음) → **[개선(merge)]**(보존+덧씌움, 충돌은 `harness-merge-advisor`로 심층 분석).

추천은 모드 프롬프트의 **기본값(엔터)**이 되고, 비대화형이면 추천을 자동 채택한다. 마커의 `mode` 필드를 읽어 혼합(merge) 이력을 구분하므로, merge로 설치한 대상을 재실행해도 커스텀이 replace로 밀리지 않는다.

## 설치 모드 — 개선(merge) vs 완전치환(replace)

`--mode`/`-Mode`로 지정(미지정+대화형이면 위 추천을 기본으로 묻는다).

| 모드 | 동작 | 충돌 처리 |
|---|---|---|
| **개선(merge)** (기존 커스텀 감지 시 추천) | 기존 위에 덧씌움. 비충돌만 추가, 동일 스킵 | 같은 이름·다른 내용은 **덮지 않고** `*.harness-incoming`으로 보존 + `harness-merge-report-*.txt` 생성 |
| **완전치환(replace)** (기존 없음 시 추천) | 기존을 전부 타임스탬프 백업 후 내 깃허브 하네스로 **전면 교체** | 전체 교체(백업 보존) |

> 기존 하네스가 없으면 자동 `replace`. 비대화형 + 기존 존재 시엔 분석 추천을 채택 — 외부 커스텀·merge 이력이면 `merge`(비파괴), 우리 `replace` 이전 설치면 `replace`(업그레이드, 기존 백업). 대화형 프롬프트에서 무효 입력은 안전하게 `merge`로 폴백한다.
