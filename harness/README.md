...
`install.sh`/`install.ps1`은 **설치를 시작하기 전에 대상 머신의 기존 하네스를 심층 분석**한다 — `CLAUDE.md`(줄수)·`rules`·`skills`·`agents`·`settings.json` 존재를 스캔해 **모드를 추천**한다:
- 기존 하네스 **없음/최소** → **[완전 치환]**(깨끗한 신규 설치).
- **내 깃허브 하네스 이전 설치 감지**(`.harness-source` 마커) → **[완전 치환=업그레이드]**(최신본 클린 교체, 기존 백업 — merge는 우리↔우리 충돌만 양산).
- **외부/커스텀 하네스 감지**(마커 없음) → **[개선(merge)]**(보존+덧씌움, 충돌은 `harness-merge-advisor`로 심층 분석).

추천은 모드 프롬프트의 **기본값(엔터)**이 되고, 비대화형이면 추천을 자동 채택한다.
...
> 기존 하네스가 없으면 자동 `replace`. 비대화형 + 기존 존재 시엔 분석 추천(보통 `merge`=비파괴)을 채택.
