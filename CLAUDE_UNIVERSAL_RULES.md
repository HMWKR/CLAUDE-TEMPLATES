...
단계별로 생각해줘.
```
> ⚠️ 너무 단순해서 깊은 사고 보장 안 됨

#### 2단계: 구조화된 사고 유도 (효과 중간)

```
복잡한 상황을 분석할 때 다음 추론 구조를 사용해줘:
1. 먼저 모든 관련 요소와 이해관계자를 파악해
2. 각 요소의 잠재적 영향과 중요성을 설명해
3. 요소들 간의 상호작용을 고려해
4. 다양한 시나리오와 확률을 검토해
5. 신뢰도 수준과 함께 근거 있는 결론을 제시해

각 단계에서 작업 과정을 보여주고, 가정이 있다면 설명해줘.
```

#### 3단계: 계층적 분해 (효과 높음)

```
이 복잡한 문제를 계층적 분해 방식으로 접근해줘:

Level 1: 주요 문제를 3-5개의 핵심 구성요소로 분해
Level 2: 각 구성요소에서 2-4개의 하위 이슈를 식별
Level 3: 각 하위 이슈에 대한 구체적 분석 단계를 설명

그런 다음 각 레벨을 체계적으로 분석하고,
하위 레벨의 통찰이 상위 레벨의 이해에 어떻게 기여하는지
명확한 연결을 유지해줘.
```

#### 4단계: XML 태그 구조화 (매우 효과적)

```
<instructions>
이 문제를 분석할 때 다음 프레임워크를 따라줘:
</instructions>

<thinking>
[여기서 네 전체 사고 과정을 자세히 보여줘]
- 초기 가설
- 반론과 대안적 관점
- 각 접근법의 장단점
- 불확실한 부분과 그 이유
...
**Thinking Mode ON일 때**:
```
이 문제에 대해 철저하고 깊이 있게 생각해줘.
여러 접근법을 고려하고 완전한 추론을 보여줘.
첫 번째 방법이 안 되면 다른 방법도 시도해봐.
```

> Claude가 자체적으로 깊은 사고를 하므로, 너무 세부적인 지시는 오히려 방해
...
이 결정을 내리기 전에:
- 3가지 이상의 대안을 먼저 생성해
- 각 대안의 2차, 3차 파급효과를 분석해
- 최악의 시나리오에서도 수용 가능한 옵션을 식별해
- 그런 다음 추천하고 그 이유를 설명해
```
...
```
이 버그를 디버깅할 때:
- 가능한 원인들을 가능성 순으로 나열해
- 각 가설을 검증할 수 있는 방법을 제시해
- 먼저 가장 가능성 높은 원인부터 조사하되,
- 초기 가설이 틀렸을 경우의 대안 경로도 미리 계획해둬
```
...
```
이 아이디어를 발전시키기 전에:
- 5가지 완전히 다른 접근법을 브레인스토밍해
- 각 접근법의 독특한 강점을 파악해
- 가장 예상치 못한 조합을 시도해봐
- 그 후에 가장 유망한 방향을 선택하고 왜 그런지 설명해
```
...
## 12-3. 프롬프트 수집 시스템 (v3.0)

16개 섹션 커밋 메시지와 **프롬프트 저널**에서 프롬프트를 자동 추출하여 대시보드에 집계하는 시스템입니다.
...
│  [소스 1] 16섹션 커밋 ────────┐                                 │
│  • commitlint 검증            │                                 │
│  • Git 히스토리에 저장        ├──▶ prompts.json ──▶ dashboard   │
│                               │                                 │
│  [소스 2] 프롬프트 저널 ──────┘                                 │
│  • .prompts/*.md 파일                                           │
...
1. **init-project.sh 실행** - 자동으로 sync-prompts.yml 워크플로우 + .prompts/ 폴더 생성
