...
**Codex-templates**는 Codex CLI 협업을 위한 **마스터 템플릿 저장소**입니다.

새로운 프로젝트에서 Codex와 효과적으로 협업하기 위한 템플릿, 규칙, 가이드라인, 자동화 설정을 한곳에 모아둔 "씨앗 저장소"입니다.

### 핵심 기능

1. **AGENTS.md 템플릿** - 새 프로젝트에 복사할 템플릿 (글로벌 참조 구조)
...
```bash
# 1. 템플릿 복사 (Windows)
copy C:\Users\jusan\Desktop\Codex-templates\CLAUDE_TEMPLATE.md .\AGENTS.md
...
# 3. 설정 파일 복사
copy C:\Users\jusan\Desktop\Codex-templates\commitlint.config.cjs .
copy C:\Users\jusan\Desktop\Codex-templates\.gitmessage .
...
├── CLAUDE_TEMPLATE.md           # AGENTS.md 전체 템플릿
├── PROJECT_SETUP_CHECKLIST.md   # 새 프로젝트 설정 체크리스트
├── CONVERSATION_PROMPTS.md      # Codex 대화 프롬프트 패턴 모음
├── PROMPT_JOURNAL_TEMPLATE.md   # 프롬프트 저널 템플릿 (v3.0)
├── CODE_REVIEW_GUIDE.md         # 코드 리뷰 가이드라인
├── CLAUDE_CODE_SKILLS_GUIDE.md  # Codex 스킬 활용 가이드
├── PLUGIN_GUIDE.md              # Codex 플러그인 가이드
├── SECTION_1_TO_8_WRITING_GUIDE.md # AGENTS.md 섹션 1-8 작성 가이드
├── archive/                     # PE 시대 레거시 보관
│   └── pe-legacy/
│       ├── 커밋메시지-16섹션-설정가이드.md  # [아카이브] PE 시대 커밋 가이드
│       ├── CLAUDE_OPTIMIZATION_ANALYSIS.md # [아카이브] 최적화 분석
│       └── CLAUDE_UNIVERSAL_RULES.md       # [아카이브] 공통 규칙 (→ 글로벌 AGENTS.md로 이관)
...
│       │   ├── SKILL.md         # frontmatter + Skill Bundle Note + 원본 AGENTS.md byte-for-byte
│       │   ├── README.md        # 출처·제외·install 안내
│       │   └── references/      # 30개 baseline-lifecycle md 원문 보존
│       │       ├── original-AGENTS.md          # 원본 byte-for-byte 사본
...
새 프로젝트의 `AGENTS.md`로 복사할 전체 템플릿입니다.
...
```bash
# 새 프로젝트에 복사
cp CLAUDE_TEMPLATE.md /path/to/new-project/AGENTS.md
...
**현재 위치**:
- `~/.Codex/AGENTS.md` — 글로벌 공통 지침
- `~/.Codex/rules/` — 환각 방지, 루프 방지, 안전 규칙
...
Codex와 효과적으로 대화하기 위한 검증된 프롬프트 패턴 모음입니다.
...
**위치**: `.Codex/skills/aidlc-baseline/`

`realizesoft/table-order-macos-Codex/` 의 baseline lifecycle 을 byte-for-byte 보존하여 자산화한 첫 프로젝트 스킬.

**구성**:
- `SKILL.md` — Codex 스킬 진입점. frontmatter + Skill Bundle Note (references/ 우선순위 안내) + 원본 `.Codex/AGENTS.md` byte-for-byte 본문
- `README.md` — 출처, 가이드 §4.7·6.4 에 따른 `requirements/` 제외 사유, install 방법, 스킬 2 부착 명시
- `references/original-AGENTS.md` — 원본 byte-for-byte 100% 보존 사본
...
**Hybrid 배치 전략**:
- **마스터 사본** (git 추적): `Codex-templates/.Codex/skills/aidlc-baseline/`
- **활성 글로벌 사본**: `~/.Codex/skills/aidlc-baseline/` — `scripts/install-aidlc-baseline.ps1` 실행으로 배포
...
**참조**:
- `realizesoft/realizesoft-cross-runtime-skill-guide.md` (§6.4 Concrete Example, §11.2 Codex, §16 Build Procedure, §18 Verification Checklist)
- `.Codex/plans/table-order-macos-Codex-requirement-iridescent-leaf.md` — 본 스킬화 작업의 plan 기록

### 4.9 aidlc-realizesoft 스킬 (v1.0)

**위치**: `.Codex/skills/aidlc-realizesoft/`
...
**Hybrid 배치 전략**:
- **마스터 사본** (git 추적): `Codex-templates/.Codex/skills/aidlc-realizesoft/`
- **활성 글로벌 사본**: `~/.Codex/skills/aidlc-realizesoft/` — `scripts/install-aidlc-realizesoft.ps1` 실행으로 배포
- **backup 격리**: `~/.Codex/skills-backups/` (skill registry 오염 방지 — 스킬 1 에서 검증된 패턴)

**사전조건**:
`aidlc-baseline` (§4.8) 이 먼저 글로벌 설치되어 있어야 함. install 스크립트가 `~/.Codex/skills/aidlc-baseline/SKILL.md` 존재 여부를 검증.
...
**참조**:
- `realizesoft/realizesoft-cross-runtime-skill-guide.md` (§3 Core Architecture, §4 Non-Negotiable Rules, §9 Helper Routing, §10 Selection Gate, §11.2 Codex command skeleton, §12 UserChoice, §13 UI/UX Gate, §14 Deployment Gate, §15 Explicit-Only Skills, §17 Minimal Template, §18 Verification Checklist)
- `.Codex/plans/table-order-macos-Codex-requirement-iridescent-leaf.md` — 본 스킬 작업의 plan 기록 (Why/What/How/So What 4단계 + 9 실행 단계)
...
이 저장소는 **CE v2.0 템플릿**만 관리한다. 사용자의 글로벌 Codex 환경(`~/.Codex/`)은 별도로 진화하며, 현재 상태는 프로젝트 메모리에 스냅샷으로 유지된다.

| 항목 | 최신 스냅샷 위치 |
|------|----------------|
| MCP 서버 / 플러그인 / 훅 / 로컬 도구 인벤토리 | `~/.Codex/projects/<slug>/memory/environment_inventory.md` (2026-04-11) |
| CapCut MCP 연동 (VectCutAPI) | `~/.Codex/projects/<slug>/memory/capcut_integration.md` |
| 하니스 업그레이드 절차 | `~/.Codex/HARNESS_UPGRADE_CHECKLIST.md` |

> **원칙**: 글로벌 환경의 구체적 도구 목록(MCP 서버, 설치 경로 등)은 `AGENTS.md`에 직접 기재하지 않는다. CE v2.0의 "최소 토큰 + 중복 제거" 원칙에 따라 `Codex mcp list` 등 실시간 조회로 대체 가능한 정보는 메모리에만 시점 기록으로 남긴다.

---

# 작업 지침

> 공통 규칙(언어, 환각 방지, 코드 작성, 보안 등)은 **글로벌 `~/.Codex/AGENTS.md`** 참조.
...
커밋 후 Codex가 `.thoughts/YYYY-MM-DD-{subject}.md`에 CE 관점 사고 과정을 기록합니다.
