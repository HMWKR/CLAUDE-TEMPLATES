...
## 처음 오신 분께 — 이 레포가 보여주는 것 (30초 요약)

AI 코딩 에이전트를 "믿고 쓰기 위해" 직접 설계·운영 중인 개인 엔지니어링 시스템입니다. 핵심은 AI가 아니라 **검증**입니다:

1. **검증 게이트** — AI 산출물은 독립 리뷰 워커(교차 벤더)의 비평과 Verification Checklist를 통과해야 채택됩니다. 승인 없는 워커 호출 금지, 이력은 append-only 로그로 남깁니다. ([`CLAUDE.md`](./CLAUDE.md) §Approval Gate·§Verification)
2. **파일 기반 계약** — 워커 입출력은 스키마가 고정된 파일(task/brief/result/log)로만 오가고, 쓰기 권한은 `write_scope`로 경로 단위 제한합니다 — 데이터 파이프라인의 스키마 계약과 같은 원리입니다. ([`_templates/`](./_templates/))
3. **재현 가능한 환경** — 스킬 50종·에이전트·MCP 22종 구성 전체를 새 Mac/Windows에 스크립트로 동일 재구성하고([`harness/`](./harness/)), 네이티브 플러그인으로 패키징해 배포합니다([`plugins/jusan-harness/`](./plugins/jusan-harness/)).

> 이 시스템으로 만든 산출물 예: [adventureworks-sales-service](https://github.com/HMWKR/adventureworks-sales-service) (데이터 수집→분석→ML 예측→라이브 서비스)
...
| **③ 하네스 플러그인 + 마켓플레이스** | [`plugins/jusan-harness/`](./plugins/jusan-harness/) · `.claude-plugin/marketplace.json` | 스킬 50·에이전트 6을 네이티브 플러그인으로 설치(`jusan-harness@claude-templates`). 기존 하네스 위 설치 시 `harness-merge-advisor` 스킬이 충돌 분석·제안 |

> 전체 하네스를 새 머신에 깔려면 → **[`harness/README.md`](./harness/README.md)** 의 Mac/Windows 설치 절차(범위 글로벌/프로젝트 · 모드 merge/replace)를 따른다.
> 스킬·에이전트는 ②(파일 복사)·③(플러그인) 중 택1로 설치한다. 정본은 `plugins/jusan-harness/`이며 ②의 install 스크립트가 거기서 복사한다(중복 없음).
> 글로벌 `~/.claude/CLAUDE.md`는 루트가 아니라 `harness/dot-claude/`에 백업한다(루트 `CLAUDE.md`는 프로젝트 전용 오케스트레이션 규칙으로 의도적 분리).
>
> **플러그인 설치**: `claude plugin marketplace add HMWKR/CLAUDE-TEMPLATES && claude plugin install jusan-harness@claude-templates`
