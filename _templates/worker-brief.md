...
[한 줄 목적 — 이 worker가 완료할 것. mat이 첫 의미줄을 워커 목적으로 표시하므로 평문으로 둔다(heading·코드펜스·인용 prefix 금지). 아래 ## Objective와 같은 내용.]

<!-- HARD LIMIT: 1200자 한글 / 240단어 영문 (wc -m / wc -w). 파일 내용 inline 금지. 경로만 전달. -->
<!-- KI-1(a) 수정: 위 평문 목적 줄이 mat의 firstMeaningfulLine이 되어 ```yaml/행동규약 불릿 오표시를 막는다. -->
